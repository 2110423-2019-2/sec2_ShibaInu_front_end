    ...
    return (<div>
      ...
      <Container>
        ...
        <Row>
          ...
          <Col sm={8}>
            <Row><FreelancerBox freelancerList={this.state.freelancerList} jobId={this.state.jobID} contract={this.state.contract}/></Row>
...
class FreelancerBox extends React.Component {
  ...
  async handleCancel(){
    axios
    .delete(utilities["backend-url"] + "/contracts/deleteByJobId/" + this.props.jobId)
    .then(res =>{
        console.log(res.data)
        this.setState({
            selected : null,
  ...
  showInterestedList() {
    return this.state.freelancerList.map(item => (
      <tr key={item.userId}>
        ...
        <td>
          {this.state.contract!==null && this.state.contract.freelancerId==item.userId?
          <button type="button" className="btn btn-danger" disabled={this.state.contract!==null && this.state.contract.freelancerId!=item.userId} name={item.userId} onClick={this.handleCancel.bind(this,)}>
            cancel
          </button>
          :
          <button type="button" className="btn btn-primary" disabled={this.state.contract!==null && this.state.contract.freelancerId!=item.userId} name={item.userId} onClick={this.handleSelect.bind(this,)}>
            select
          </button>
          }
        </td>
...
class DashboardContract extends React.Component {
  ...
  getContractComponent() {
    return (
      <button type="button" className="btn btn-warning" onClick={this.gotoContract.bind(this,)}>
        SHOW CONTRACT
      </button>
    )
  }
